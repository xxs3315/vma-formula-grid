...
const DistributionFunctions = {
  ...
  'BINOM.DIST.RANGE': (trials: any, probabilityS: any, numberS: any, numberS2: any) => {
    ...
    if (trials < 0 || probabilityS < 0 || probabilityS > 1 || numberS < 0 || numberS > trials || numberS2 < numberS || numberS2 > trials) throw FormulaError.NUM
  ...
  'CHISQ.TEST': (actualRange: any, expectedRange: any) => {
    ...
    if (actual.length !== expected.length || actual[0].length !== expected[0].length || (actual.length === 1 && actual[0].length === 1)) throw FormulaError.NA
  ...
  'FORECAST.LINEAR': (x: any, knownYs: any, knownXs: any) => DistributionFunctions.FORECAST(x, knownYs, knownXs),
  ...
  'HYPGEOM.DIST': (sample_s: any, number_sample: any, population_s: any, number_pop: any, cumulative: any) => {
