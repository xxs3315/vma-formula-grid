...
const ReferenceFunctions = {
  ...
  HLOOKUP: (lookupValue: any, tableArray: any, rowIndexNum: any, rangeLookup: any) => {
    ...
    if (WildCard.isWildCard(lookupValue)) {
      index = tableArray[0].findIndex((item: any) => WildCard.toRegex(lookupValue, 'i').test(item))
    } else {
  ...
  INDEX: (context: any, ranges: any, rowNum: any, colNum: any, areaNum: any) => {
    ...
    if (rowNum !== 0 && colNum !== 0) {
      ...
      if (H.isRangeRef(range)) {
        range = range.ref
        if (range.to.row - range.from.row < rowNum - 1 || range.to.col - range.from.col < colNum - 1) throw FormulaError.REF
        return {
          ref: {
  ...
  VLOOKUP: (lookupValue: any, tableArray: any, colIndexNum: any, rangeLookup: any) => {
    ...
    let index = -1
    if (WildCard.isWildCard(lookupValue)) {
      index = tableArray.findIndex((currRow: any) => WildCard.toRegex(lookupValue, 'i').test(currRow[0]))
    } else {
