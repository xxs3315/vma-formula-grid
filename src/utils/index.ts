...
export function getRenderDefaultColWidth(
	defaultColumnWidth: number | undefined,
	gridSize: VmaFormulaGridPropTypes.Size,
): number {
...
export function getRenderDefaultRowHeight(
...
export function getRenderRowIndicatorWidth(
	gridSize: VmaFormulaGridPropTypes.Size,
): number {
	// if (gridSize === 'xxx-large') {
	//     return 20
...
export const getIndexFromColumnWidths = (
	...
	if (
		...
		Object.keys(changedColumnHides).length
	) {
		// 配置中既有列宽定义，又有列隐藏定义
		let sidx = 0;
		let x = 0;
...
export const getRealVisibleWidthSize = (
	...
	while (xSize < viewportWidth) {
		xSize +=
			changedColumnHides[`${visibleIndex + x + 1}`] === 0
				? 0
				: changedColumnWidths[`${visibleIndex + x + 1}`]
				? changedColumnWidths[`${visibleIndex + x + 1}`]
				: colWidth;
		x++;
	}
...
export const getIndexFromRowHeights = (
	...
	if (
		...
		Object.keys(changedRowHides).length
	) {
		// 配置中既有行高定义，又有行隐藏定义
		let sidy = 0;
		let y = 0;
...
export const getRealVisibleHeightSize = (
	...
	while (ySize < viewportHeight) {
		ySize +=
			changedRowHides[`${visibleIndex + y + 1}`] === 0
				? 0
				: changedRowWidths[`${visibleIndex + y + 1}`]
				? changedRowWidths[`${visibleIndex + y + 1}`]
				: rowHeight;
		y++;
	}
...
export const getYSpaceFromRowHeights = (
	...
	if (Object.keys(changedRowHeights).length) {
		...
		while (startIndex > sidx) {
			ySpace += changedRowHeights[`${sidx + 1}`]
				? changedRowHeights[`${sidx + 1}`]
				: rowHeight;
			sidx++;
		}
		return ySpace;
...
export const getWidth = (
	...
	if (
		...
		for (const k in Object.keys(changedColumnHides)) {
			if (
				changedColumnHides.hasOwnProperty(Object.keys(changedColumnHides)[k])
			) {
...
export class ll<nd = any> {
	...
	): ll<nd | Record<string | number | symbol, unknown>> {
		const list = new ll();
		let finish = end;
	...
	public reduce(
		f: (accumulator: any, currentNode: nd, index: number, list: ll<nd>) => any,
		start?: any,
		reverse = false,
	): any {
		let currentIndex = reverse ? this.length - 1 : 0;
...
export class d {
	constructor(g: Record<string, any>) {
		...
		this._e = 0;
		this._aj = ll.from([
			...Array.from({ length: this._v }, () => new ll<number>()),
		]);
...
export default class dc {
	...
	hc = (): boolean =>
		this._cy !== null && this._cy !== undefined && this._cy.length > 0;
	...
	if (dcs.hc()) {
		for (let i = 0; i < dcs.c.length; i++) {
			if (!cycleVertexes.hasOwnProperty(vertexKeys[dcs.c[i]])) {
				cycleVertexes[vertexKeys[dcs.c[i]]] = [
					vertexes[vertexKeys[dcs.c[i]]],
				].concat()[0];
...
export function getRowColSpanFromMerges(
	col: number,
	row: number,
...
export function checkCellInMerges(
	col: number,
	row: number,
	merges: Record<string, any>,
): boolean {
	...
	for (let i = 0; i < keys.length; i++) {
		if (
			col > merges[keys[i]].colStart &&
			col <= merges[keys[i]].colEnd &&
			row >= merges[keys[i]].rowStart &&
			row <= merges[keys[i]].rowEnd
		) {
			return true;
		}
		if (
...
export function getRealArea(
	...
	// TODO problem??
	if (
		currentArea.start.row === endRowIndex &&
		currentArea.start.col === endColIndex
	) {
	...
	if (startColIndex === endColIndex && startRowIndex === endRowIndex) {
		...
		Object.keys(merges).forEach((key: string) => {
			...
			let startCol = [
				Math.min(startColIndex + 1, endColIndex + 1),
				Math.min(merges[key].colStart, merges[key].colEnd),
			];
			let endCol = [
				Math.max(startColIndex + 1, endColIndex + 1),
				Math.max(merges[key].colStart, merges[key].colEnd),
			];
			if (Math.max(...startCol) <= Math.min(...endCol)) {
				mergesIntersectCol = true;
			}
			let startRow = [
				Math.min(startRowIndex + 1, endRowIndex + 1),
				Math.min(merges[key].rowStart, merges[key].rowEnd),
			];
			let endRow = [
				Math.max(startRowIndex + 1, endRowIndex + 1),
				Math.max(merges[key].rowStart, merges[key].rowEnd),
			];
		...
		if (mergeColRows.length > 0) {
			result.w = getCurrentAreaWidth(
				startColIndex,
				startColIndex + startColSpan - 1,
				columnWidth,
				changedColumnWidths,
				changedColumnVisibles,
			);
			result.h = getCurrentAreaHeight(
				startRowIndex,
		...
		while (mergesUnComparedKeys.length > 0) {
			...
			let mergesIntersectRow = false;
			let startCol = [
				Math.min(sci + 1, eci + 1),
				Math.min(
					merges[mergesUnComparedKeys[0]].colStart,
					merges[mergesUnComparedKeys[0]].colEnd,
				),
			];
			let endCol = [
				Math.max(sci + 1, eci + 1),
				Math.max(
					merges[mergesUnComparedKeys[0]].colStart,
					merges[mergesUnComparedKeys[0]].colEnd,
				),
			...
			let startRow = [
				Math.min(sri + 1, eri + 1),
				Math.min(
					merges[mergesUnComparedKeys[0]].rowStart,
					merges[mergesUnComparedKeys[0]].rowEnd,
				),
			];
			let endRow = [
				Math.max(sri + 1, eri + 1),
				Math.max(
					merges[mergesUnComparedKeys[0]].rowStart,
					merges[mergesUnComparedKeys[0]].rowEnd,
				),
...
export function calcXOverlapMerges(
	offsetStartColIndex: number,
	merges: Record<string, any>,
	direction: string,
) {
	Object.keys(merges).forEach((key: string) => {
...
export function calcYOverlapMerges(
	...
	Object.keys(merges).forEach((key: string) => {
		if (
			offsetStartIndex + 1 > merges[key].rowStart &&
			offsetStartIndex + 1 < merges[key].rowEnd
		) {
			if (direction === "min") {
					...
					item.p.forEach((pos: string) => {
						if (pos.indexOf(":") >= 0) {
							...
							if (
								...
								bg =
									!item.hasOwnProperty("color") || item.color === "none"
										? ""
										: item.color;
							}
						} else {
...
export function calcCellBorders(
	colIndex: number,
	rowIndex: number,
	borders: any[],
	colLength: number,
	...
	if (borders && borders.length > 0) {
				...
				if (item.p.indexOf(getColumnSymbol(colIndex + 1)) >= 0) {
					if (item.hasOwnProperty("details")) {
						if (item.details.hasOwnProperty("none") && item.details.none) {
							...
							item.details.hasOwnProperty("full") &&
							item.details.full
						) {
							bdls.push(true);
							bdts.push(true);
							...
							item.details.hasOwnProperty("outer") &&
							item.details.outer
						) {
							bdls.push(true);
							...
							if (
								item.details.hasOwnProperty("bottom") &&
								item.details.bottom
							) {
								bdbs.push(true);
							}
						...
						if (typeof i === "string" && i.indexOf(":") >= 0) {
							...
							if (colIndex + 1 >= colStart && colIndex + 1 <= colEnd) {
								if (item.details.hasOwnProperty("none") && item.details.none) {
									...
									if (
										item.details.hasOwnProperty("left") &&
										item.details.left
									) {
										bdls.push(true);
									}
									...
									if (
										item.details.hasOwnProperty("right") &&
										item.details.right
									) {
										bdrs.push(true);
									}
					...
					if (
						...
						if (item.hasOwnProperty("details")) {
							if (item.details.hasOwnProperty("none") && item.details.none) {
								...
								(item.details.hasOwnProperty("inner") &&
									item.details.inner &&
									item.details.hasOwnProperty("outer") &&
									item.details.outer)
							) {
								...
								if (
									...
									if (
										item.details.hasOwnProperty("top") &&
										item.details.top &&
										rowIndex === Math.min(rowStart - 1, rowEnd - 1)
									) {
										bdts.push(true);
		...
		if (bdls.length > 0) {
			if (bdls.lastIndexOf("none") >= 0) {
				bdls =
					bdls.lastIndexOf("none") === bdls.length - 1
						? []
						: bdls.slice(bdls.lastIndexOf("none") + 1);
			}
			if (bdls.length > 0) {
		...
		if (bdts.length > 0) {
			if (bdts.lastIndexOf("none") >= 0) {
				bdts =
					bdts.lastIndexOf("none") === bdts.length - 1
						? []
						: bdts.slice(bdts.lastIndexOf("none") + 1);
			}
			if (bdts.length > 0) {
		...
		if (bdrs.length > 0) {
			if (bdrs.lastIndexOf("none") >= 0) {
				bdrs =
					bdrs.lastIndexOf("none") === bdrs.length - 1
						? []
						: bdrs.slice(bdrs.lastIndexOf("none") + 1);
			}
			if (bdrs.length > 0) {
		...
		if (bdbs.length > 0) {
			if (bdbs.lastIndexOf("none") >= 0) {
				bdbs =
					bdbs.lastIndexOf("none") === bdbs.length - 1
						? []
						: bdbs.slice(bdbs.lastIndexOf("none") + 1);
			}
			if (bdbs.length > 0) {
...
export function calcCellBgType(
	hasBg: boolean,
	hasBdl: boolean,
	hasBdt: boolean,
	hasBdr: boolean,
