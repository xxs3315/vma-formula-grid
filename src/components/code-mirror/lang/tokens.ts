...
const backSlash = 92,
    ...
    isSpace = (charCode: number): boolean => {
        return [9, 10, 11, 12, 13, 32, 133, 160, 5760, 8192, 8193, 8194, 8195, 8196, 8197, 8198, 8199, 8200, 8201, 8202, 8232, 8233, 8239, 8287, 12288].indexOf(charCode) !== -1;
    };
...
export const isIntersecop = new ExternalTokenizer(
    (input: InputStream, stack: Stack) => {
        ...
        if (next === openParen || isAsciiLeter(next) || isDigit(next) || before === backSlash || before === underscore) return input.acceptToken(tokens.intersecop);
    },
    { contextual: true },
);

export const decimalSeparator = new ExternalTokenizer((input: InputStream, stack: Stack) => {
    const { next } = input,
        charCodeDecimalSeparator = {
            '.': dot,
            ',': comma,
        }[currDecimalSeparator];
    if (next === charCodeDecimalSeparator) return input.acceptToken(tokens.decimalSeparator, 1);
});

export const Separator = new ExternalTokenizer((input: InputStream, stack: Stack) => {
    const { next } = input,
...
export const desambiguateNameToken = (value: string, stack: Stack): number => {
    if ((i18n.BoolToken[currIdiom] as string[]).indexOf(value.toUpperCase()) !== -1) {
...
export const isErrors = (value: string, stack: Stack): number => {
    if ((i18n.RefErrorToken[currIdiom] as string[]).indexOf(value.toUpperCase()) !== -1) {
        return tokens['RefErrorToken'];
    } else if ((i18n.ErrorToken[currIdiom] as string[]).indexOf(value.toUpperCase()) !== -1) {
        return tokens['ErrorToken'];
    }
    return -1;
};

export const setLezerIdiom = (newIdiom: TsupportedIdioms): TsupportedIdioms => (currIdiom = newIdiom);

export const setDecimalSeparator = (newDecimalSeparator: TdecimalSeparator): TdecimalSeparator => (currDecimalSeparator = newDecimalSeparator);
