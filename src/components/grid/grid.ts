...
export default defineComponent({
	...
	setup(props, context) {
		...
		const FormulaGridHeaderComponent = resolveComponent(
			"VmaFormulaGridHeader",
		) as ComponentOptions;
		const FormulaGridBodyComponent = resolveComponent(
			"VmaFormulaGridBody",
		) as ComponentOptions;
		...
		const refGridHeaderLeftFixedTableColgroup =
			ref() as Ref<HTMLTableColElement>;
		...
		const renderDefaultColWidth = computed(() =>
			getRenderDefaultColWidth(props.defaultColumnWidth, props.size),
		);

		const renderDefaultRowHeight = computed(() =>
			getRenderDefaultRowHeight(props.defaultRowHeight, props.size),
		);

		const rowIndicatorElWidth = computed(() =>
			Math.max(
				getRenderRowIndicatorWidth(props.size) +
					gridReactiveData.yEnd.toString().length *
						getRenderRowIndicatorWidth(props.size),
				54,
			),
		);
		...
		const gridMethods = {
			calc: () => {
								...
								if (
									...
									for (let r = fdItem.from.row; r <= fdItem.to.row; r++) {
										for (let c = fdItem.from.col; c <= fdItem.to.col; c++) {
											...
											if (
												vertexes[
													`${item.col + 1}_${item.row + 1}`
												].children.indexOf(`${c}_${r}`) < 0
											) {
												vertexes[
													`${item.col + 1}_${item.row + 1}`
												].children.push(`${c}_${r}`);
									...
									if (!vertexes.hasOwnProperty(`${fdItem.col}_${fdItem.row}`)) {
										if (
											errorKeyList.indexOf(`${fdItem.col}_${fdItem.row}`) < 0
										) {
									...
									if (
										vertexes[
											`${item.col + 1}_${item.row + 1}`
										].children.indexOf(`${fdItem.col}_${fdItem.row}`) < 0
									) {
										vertexes[`${item.col + 1}_${item.row + 1}`].children.push(
											`${fdItem.col}_${fdItem.row}`,
										);
									}
								}
							});
				...
				if (errorMapKeys.length > 0) {
					for (let i = 0; i < errorMapKeys.length; i++) {
						if (gridReactiveData.cells.eMap[errorMapKeys[i]].ref.se !== null) {
							gridReactiveData.cells.eMap[errorMapKeys[i]].ref.mv =
								gridReactiveData.cells.eMap[errorMapKeys[i]].ref.se;
						} else {
							gridReactiveData.cells.eMap[errorMapKeys[i]].ref.mv =
								"#REFERROR!";
							gridReactiveData.cells.eMap[errorMapKeys[i]].ref.se =
				...
				const parser = new FormulaParser({
					...
					onCell: (ref: any) =>
						checkCellInMerges(ref.col, ref.row, gridReactiveData.merges)
							? null
		...
		const gridPrivateMethods = {
			...
			mergeCells: () => {
				if (
					$vmaFormulaGrid.reactiveData.currentAreaSci ===
						$vmaFormulaGrid.reactiveData.currentAreaEci &&
					$vmaFormulaGrid.reactiveData.currentAreaSri ===
						$vmaFormulaGrid.reactiveData.currentAreaEri
				) {
					return;
				}
				const key =
				...
				$vmaFormulaGrid.reactiveData.currentSheetData[
					$vmaFormulaGrid.reactiveData.currentAreaSri
				][$vmaFormulaGrid.reactiveData.currentAreaSci + 1].colSpan =
					value.colSpan;
				$vmaFormulaGrid.reactiveData.currentSheetData[
					$vmaFormulaGrid.reactiveData.currentAreaSri
				][$vmaFormulaGrid.reactiveData.currentAreaSci + 1].rowSpan =
					value.rowSpan;
				nextTick(() => {
					$vmaFormulaGrid.calcCurrentCellEditorStyle();
					$vmaFormulaGrid.calcCurrentCellEditorDisplay();
			...
			unmergeCells: () => {
				...
				Object.keys($vmaFormulaGrid.reactiveData.merges).map((key: string) => {
					...
					if (
						...
						reservedMerges[key] = Object.assign(
							{},
							$vmaFormulaGrid.reactiveData.merges[key],
						);
					} else {
						removedMerges[key] = Object.assign(
							{},
							$vmaFormulaGrid.reactiveData.merges[key],
						);
					}
				...
				Object.keys(removedMerges).map((key) => {
					...
					let cellStartCol = Number(cellStartColRow[0]);
					let cellStartRow = Number(cellStartColRow[1]);
					$vmaFormulaGrid.reactiveData.currentSheetData[cellStartRow - 1][
						cellStartCol - 1 + 1
					].colSpan = 1;
					$vmaFormulaGrid.reactiveData.currentSheetData[cellStartRow - 1][
						cellStartCol - 1 + 1
			...
			reCalcCurrentAreaPos: () => {
				if (
					...
					const { w, h, sci, eci, sri, eri, startRowIndex, startColIndex } =
						getRealArea(
			...
			updateCurrentAreaStyle: () => {
				if (
					...
					nextTick(() => {
						const cellElems: NodeListOf<any> =
							refGridBodyTable.value.querySelectorAll(
						...
						if (cellElems.length > 0) {
							const borderMarginLeft =
								leftSpaceWidth + cellElems[0].offsetLeft - 1;
							const borderMarginTop =
								topSpaceHeight + cellElems[0].offsetTop - 1;
			...
			calcCurrentCellEditorStyle: () => {
				if (gridReactiveData.currentCell) {
					nextTick(() => {
						...
						const cells = refGridBodyTable.value.querySelectorAll(
						...
						if (cells.length > 0) {
							cells.forEach((cellElem: any) => {
								...
								gridReactiveData.currentCellEditorStyle.height = `${
									cellElem.offsetHeight - 1
								}px`;
								gridReactiveData.currentCellEditorStyle.width = `${
				...
				gridReactiveData.currentSheetData.map((row: Cell[], index: number) => {
					row.map((cell: Cell) => {
						if (
							...
							cell.colSpan > 1 &&
							cell.col < colNumber &&
							cell.col + cell.colSpan >= colNumber
						) {
							cell.colSpan += 1;
						}
				...
				gridReactiveData.currentSheetData.map((row: Cell[], _: number) => {
					row.map((cell: Cell) => {
						if (
							...
							cell.rowSpan > 1 &&
							cell.row < rowNumber &&
							cell.row + cell.rowSpan >= rowNumber
						) {
							cell.rowSpan += 1;
						}
				...
				if (rowNumber + 1 <= gridReactiveData.currentSheetData.length) {
					gridReactiveData.currentSheetData[rowNumber].map((cell: Cell) => {
						if (cell.rowSpan && cell.rowSpan > 1) {
							gridReactiveData.currentSheetData[rowNumber + 1][
								cell.col + 1
			...
			setCellBorder: (
				...
				target: "l" | "t" | "r" | "b" | "none" | "full" | "outer" | "inner",
			) => {
				if (type === "cells") {
					const pt: any = {};
					if (target === "none") {
					...
					for (
						...
						for (
							...
							const {
								...
								col,
								row,
								gridReactiveData.borders,
								gridReactiveData.colConfs.length,
								gridReactiveData.rowConfs.length,
							);
							$vmaFormulaGrid.reactiveData.currentSheetData[row][col + 1].bgt =
								calcCellBgType(
									bg.length > 0,
									bdlCurrent,
									bdtCurrent,
				...
				if (type === "cells") {
					...
					for (
						...
						for (
							...
							$vmaFormulaGrid.reactiveData.currentSheetData[row][col + 1].bg =
								bg;
							$vmaFormulaGrid.reactiveData.currentSheetData[row][col + 1].bgt =
								calcCellBgType(
									bg.length > 0,
				...
				if (type === "cells") {
					...
					for (
						...
						for (
							...
							const { fg } = calcCellStyles(
								col,
								row,
								$vmaFormulaGrid.reactiveData.styles,
							);
							$vmaFormulaGrid.reactiveData.currentSheetData[row][col + 1].fg =
								fg;
						}
					}
				}
					...
					if (mode === "fontBold") {
						...
						for (
							...
							for (
								...
								const { b } = calcCellStyles(
									col,
									row,
									$vmaFormulaGrid.reactiveData.styles,
								);
								$vmaFormulaGrid.reactiveData.currentSheetData[row][col + 1].b =
									b;
							}
						}
					}
					if (mode === "fontItalic") {
						...
						for (
							...
							for (
								...
								const { i } = calcCellStyles(
									col,
									row,
									$vmaFormulaGrid.reactiveData.styles,
								);
								$vmaFormulaGrid.reactiveData.currentSheetData[row][col + 1].i =
									i;
							}
						}
					}
					...
					if (Object.keys(gridReactiveData.columnHidesChanged).length) {
						if (colStart === colEnd) {
							...
							while (
								...
								gridReactiveData.colConfs[Number(colStart) - idx].visible =
									true;
					...
					if (Object.keys(gridReactiveData.rowHidesChanged).length) {
						if (rowStart === rowEnd) {
							...
							while (
								...
								gridReactiveData.rowConfs[Number(rowStart) - idx - 1].visible =
									true;
		...
		const updateCurrentCell = () => {
			if (gridReactiveData.currentCell) {
				if (
					gridReactiveData.currentCell.row < 0 ||
					gridReactiveData.currentCell.col < 0
				) {
					gridReactiveData.currentCell = null;
				}
			}
			...
			if (type === "deleteColumn") {
				...
				Object.keys(gridReactiveData.merges).map((key) => {
					...
					if (Number(crStartArr[0]) > col!) {
						...
						Number(crStartArr[0]) <= col! &&
						col! < Number(crEndArr[0])
					) {
						crEndArr[0] = (Number(crEndArr[0]) - 1).toString();
					}
				...
				if ($vmaFormulaGrid.reactiveData.styles) {
					if (
						$vmaFormulaGrid.reactiveData.styles &&
						$vmaFormulaGrid.reactiveData.styles.bgc &&
						$vmaFormulaGrid.reactiveData.styles.bgc.length > 0
					) {
						const bgcNew: Record<string, any>[] = [];
						$vmaFormulaGrid.reactiveData.borders.forEach((bgcItem: any) => {
							if (
								bgcItem.hasOwnProperty("type") &&
								(bgcItem.type === "columns" || bgcItem.type === "cells")
							) {
								if (
									bgcItem.type === "columns" &&
									bgcItem.hasOwnProperty("p") &&
									bgcItem.p.length > 0
								) {
									const posTemp: any[] = [];
									bgcItem.p.forEach((bgcItemPos: string) => {
										if (bgcItemPos.indexOf(":") >= 0) {
											...
											if (columnEnd >= columnStart) {
												posTemp.push(
													getColumnSymbol(columnStart) +
														":" +
														getColumnSymbol(columnEnd),
												);
											}
										} else {
											if (col! + 1 < getColumnCount(bgcItemPos)) {
												posTemp.push(
													getColumnSymbol(getColumnCount(bgcItemPos) - 1),
												);
								...
								if (
									...
									if (bgcItem.p.indexOf(":") >= 0) {
										...
										let cellPrevRow = parseInt(
											cellPrev.replace(/[^0-9]/gi, ""),
										);
										let cellNextColStr = cellNext.replace(/[0-9]/g, "");
										let cellNextRow = parseInt(
											cellNext.replace(/[^0-9]/gi, ""),
										);
										if (
											col! + 1 <
											Math.min(
												getColumnCount(cellPrevColStr),
												getColumnCount(cellNextColStr),
											)
										) {
											cellPrevColStr = getColumnSymbol(
												getColumnCount(cellPrevColStr) - 1,
											);
											cellNextColStr = getColumnSymbol(
												getColumnCount(cellNextColStr) - 1,
											);
										} else if (
											(col! + 1 >= getColumnCount(cellPrevColStr) &&
												col! + 1 <= getColumnCount(cellNextColStr)) ||
											(col! + 1 >= getColumnCount(cellNextColStr) &&
												col! + 1 <= getColumnCount(cellPrevColStr))
										) {
											if (
										...
										if (col! + 1 < getColumnCount(cellColStr)) {
											cellColStr = getColumnSymbol(
												getColumnCount(cellColStr) - 1,
											);
											bgcNew.push(
												Object.assign({}, bgcItem, { p: cellColStr + cellRow }),
											);
					...
					if (
						$vmaFormulaGrid.reactiveData.styles &&
						$vmaFormulaGrid.reactiveData.styles.fgc &&
						$vmaFormulaGrid.reactiveData.styles.fgc.length > 0
					) {
						const fgcNew: Record<string, any>[] = [];
						$vmaFormulaGrid.reactiveData.borders.forEach((fgcItem: any) => {
							if (
								fgcItem.hasOwnProperty("type") &&
								(fgcItem.type === "columns" || fgcItem.type === "cells")
							) {
								if (
									fgcItem.type === "columns" &&
									fgcItem.hasOwnProperty("p") &&
									fgcItem.p.length > 0
								) {
									const posTemp: any[] = [];
									fgcItem.p.forEach((fgcItemPos: string) => {
										if (fgcItemPos.indexOf(":") >= 0) {
											...
											if (col! + 1 < getColumnCount(fgcItemPos)) {
												posTemp.push(
													getColumnSymbol(getColumnCount(fgcItemPos) - 1),
												);
								...
								if (
									...
									if (fgcItem.p.indexOf(":") >= 0) {
										...
										if (col! + 1 < getColumnCount(cellColStr)) {
											...
											fgcNew.push(
												Object.assign({}, fgcItem, { p: cellColStr + cellRow }),
											);
										} else if (col! + 1 > getColumnCount(cellColStr)) {
											fgcNew.push(
												Object.assign({}, fgcItem, { p: cellColStr + cellRow }),
				...
				// borders
				if (
					$vmaFormulaGrid.reactiveData.borders &&
					$vmaFormulaGrid.reactiveData.borders.length > 0
				) {
					const bordersNew: Record<string, any>[] = [];
					$vmaFormulaGrid.reactiveData.borders.forEach((borderItem: any) => {
						if (
							borderItem.hasOwnProperty("type") &&
							(borderItem.type === "columns" || borderItem.type === "cells")
						) {
							if (
								borderItem.type === "columns" &&
								borderItem.hasOwnProperty("p") &&
								borderItem.p.length > 0
							) {
								const posTemp: any[] = [];
								borderItem.p.forEach((borderItemPos: string) => {
									if (borderItemPos.indexOf(":") >= 0) {
										...
										if (col! + 1 < getColumnCount(borderItemPos)) {
											posTemp.push(
												getColumnSymbol(getColumnCount(borderItemPos) - 1),
											);
								...
								if (posTemp.length > 0) {
									bordersNew.push(
										Object.assign({}, borderItem, { p: posTemp }),
									);
								}
							}
							if (
								borderItem.type === "cells" &&
								borderItem.hasOwnProperty("p") &&
								borderItem.p.length > 0
							) {
				...
				if (
					...
					if (start.col === col || end.col === col) {
						if (start.col === end.col) {
							...
							if (start.col === col) {
								$vmaFormulaGrid.reactiveData.currentArea.start =
									$vmaFormulaGrid.reactiveData.currentSheetData[start.row][
										start.col + 1 + 1
									];
							}
							if (end.col === col) {
								$vmaFormulaGrid.reactiveData.currentArea.end =
									$vmaFormulaGrid.reactiveData.currentSheetData[end.row][
										end.col + 1 - 1
									];
							}
						} else if (start.col > end.col) {
			...
			if (type === "deleteRow") {
				...
				Object.keys(gridReactiveData.rowHeightsChanged).map((key) => {
					if (Number(key) > row!) {
						...
						gridRowsHeightChangedNew[newKey] =
							gridReactiveData.rowHeightsChanged[key];
					} else {
				...
				Object.keys(gridReactiveData.merges).map((key) => {
					...
					if (Number(crStartArr[1]) > row! + 1) {
						...
						Number(crStartArr[1]) <= row! + 1 &&
						row! + 1 <= Number(crEndArr[1])
					) {
						crEndArr[1] = (Number(crEndArr[1]) - 1).toString();
					}
				...
				if ($vmaFormulaGrid.reactiveData.styles) {
					if (
						...
						$vmaFormulaGrid.reactiveData.styles.bgc.forEach((bgcItem: any) => {
							if (
								bgcItem.hasOwnProperty("type") &&
								(bgcItem.type === "rows" || bgcItem.type === "cells")
							) {
								if (
									bgcItem.type === "rows" &&
									bgcItem.hasOwnProperty("p") &&
									...
									bgcItem.p.forEach((bgcItemPos: string | number) => {
										if (
											typeof bgcItemPos === "string" &&
											bgcItemPos.indexOf(":") >= 0
										) {
								...
								if (
									...
									if (bgcItem.p.indexOf(":") >= 0) {
										...
										if (row! + 1 < Math.min(cellPrevRow, cellNextRow)) {
											...
											(row! + 1 >= cellPrevRow && row! + 1 <= cellNextRow) ||
											(row! + 1 >= cellNextRow && row! + 1 <= cellPrevRow)
										) {
					...
					if (
						...
						$vmaFormulaGrid.reactiveData.styles.fgc.forEach((fgcItem: any) => {
							if (
								fgcItem.hasOwnProperty("type") &&
								(fgcItem.type === "rows" || fgcItem.type === "cells")
							) {
								if (
									fgcItem.type === "rows" &&
									fgcItem.hasOwnProperty("p") &&
									...
									fgcItem.p.forEach((fgcItemPos: string | number) => {
										if (
											typeof fgcItemPos === "string" &&
											fgcItemPos.indexOf(":") >= 0
										) {
				...
				if (
					...
					$vmaFormulaGrid.reactiveData.borders.forEach((borderItem: any) => {
						if (
							borderItem.hasOwnProperty("type") &&
							(borderItem.type === "rows" || borderItem.type === "cells")
						) {
							if (
								borderItem.type === "rows" &&
								borderItem.hasOwnProperty("p") &&
								...
								borderItem.p.forEach((borderItemPos: string | number) => {
									if (
										typeof borderItemPos === "string" &&
										borderItemPos.indexOf(":") >= 0
									) {
				...
				if (
					...
					if (start.row === row || end.row === row) {
						if (start.row === end.row) {
							...
							if (start.row === row) {
								$vmaFormulaGrid.reactiveData.currentArea.start =
									$vmaFormulaGrid.reactiveData.currentSheetData[start.row + 1][
										start.col + 1
									];
							}
							if (end.row === row) {
								$vmaFormulaGrid.reactiveData.currentArea.end =
									$vmaFormulaGrid.reactiveData.currentSheetData[end.row - 1][
										end.col + 1
									];
							}
						} else if (start.row > end.row) {
			...
			if (type === "insertRow") {
				...
				Object.keys(gridReactiveData.merges).map((key) => {
					...
					if (Number(crStartArr[1]) > row!) {
						...
						Number(crStartArr[1]) <= row! &&
						row! < Number(crEndArr[1])
					) {
						crEndArr[1] = (Number(crEndArr[1]) + 1).toString();
					}
					mergesNew[`${crStartArr.join("_") + ":" + crEndArr.join("_")}`] = {
				...
				[
					...
					h(
						...
						) as ComponentOptions,
						{
							...
							onChange: (color: any) => {
								if (
									gridReactiveData.colorPickerStore.selected &&
									gridReactiveData.colorPickerStore.selected.code ===
										"backgroundColor"
								) {
									if (color.mode && color.mode === "transparent") {
										$vmaFormulaGrid.setBackgroundColor("cells", "none", null);
									} else {
										$vmaFormulaGrid.setBackgroundColor(
											"cells",
											"normal",
			...
			if ($vmaFormulaGrid.closeMenu) {
				...
				if (
					...
					!(
						...
						DomTools.getEventTargetNode(event, colorPicker).flag
					)
				) {
					$vmaFormulaGrid.closeMenu();
				}
		...
		const calcScrollSizeX = (scrollBodyElem: HTMLDivElement): Promise<void> =>
			new Promise((resolve): void => {
				...
				if (props.virtualScrollX) {
					...
					if (
						Math.abs(offsetEndColIndex - visibleIndex - visibleSize) / 2 > 5 ||
						Math.abs(visibleIndex - offsetStartColIndex) / 2 > 5
					) {
						gridReactiveData.xStart = offsetStartColIndex;
						gridReactiveData.xEnd = offsetEndColIndex;
			...
			new Promise((resolve): void => {
				...
				if (props.virtualScrollY) {
					...
					const offsetItem = {
						...
						endIndex: Math.min(
							visibleIndex + visibleSize + 20,
							gridReactiveData.rowConfs.length - 1,
						),
					};

					let { startIndex: offsetStartIndex, endIndex: offsetEndIndex } =
						offsetItem;

					offsetStartIndex = Math.min(
						offsetStartIndex,
						calcYOverlapMerges(
							offsetStartIndex,
							$vmaFormulaGrid.reactiveData.merges,
					...
					if (
						Math.abs(offsetEndIndex - visibleIndex - visibleSize) / 2 > 5 ||
						Math.abs(visibleIndex - offsetStartIndex) / 2 > 5
					) {
						gridReactiveData.yStart = offsetStartIndex;
						gridReactiveData.yEnd = offsetEndIndex;
		...
		const updateScrollXYSpace = () => {
			...
			if (refGridHeaderTable.value) {
				...
				refGridHeaderTable.value.style.width = `${
					gridReactiveData.gridWidth + gridReactiveData.scrollbarWidth
				}px`;
			}
		};
		...
		const arrangeColumnWidth = () => {
			...
			for (
				let index = gridReactiveData.xStart;
				index <= gridReactiveData.xEnd;
				index++
			) {
		...
		const updateStyle = () => {
			...
			gridReactiveData.gridWidth = Math.min(
				gridReactiveData.gridWidth,
				gridDivClientWidth,
			);
			...
			if (
				refGridHeaderTableColgroup.value.children &&
				refGridHeaderTableColgroup.value.children.length
			) {
				Array.from(refGridHeaderTableColgroup.value.children).forEach(
					(colgroupElem: any, _: number) => {
						...
						if (idx === -1) {
							...
							colgroupElem.style.width = `${
								colConfs[idx + 1].visible
									? typeof colConfs[idx + 1].width === "string"
										? renderDefaultColWidth.value
										: colConfs[idx + 1].width
									: 0
							}px`;
						} else {
				...
				Array.from(refGridBodyTableColgroup.value.children).forEach(
					(colgroupElem: any, _: number) => {
						const idx = parseInt(colgroupElem.attributes.idx.value);
						if (idx === -1) {
							colgroupElem.style.width = `${rowIndicatorElWidth.value}px`;
			...
			if (
				refGridHeaderLeftFixedTableColgroup.value.children &&
				refGridHeaderLeftFixedTableColgroup.value.children.length
			) {
				Array.from(refGridHeaderLeftFixedTableColgroup.value.children).forEach(
					(colgroupElem: any, _: number) => {
						const idx = parseInt(colgroupElem.attributes.idx.value);
						if (idx === -1) {
							colgroupElem.style.width = `${rowIndicatorElWidth.value}px`;
				...
				Array.from(refGridBodyLeftFixedTableColgroup.value.children).forEach(
					(colgroupElem: any, _: number) => {
						const idx = parseInt(colgroupElem.attributes.idx.value);
						if (idx === -1) {
							colgroupElem.style.width = `${rowIndicatorElWidth.value}px`;
			...
			refGridHeaderLeftFixedTable.value.style.width = `${
				gridReactiveData.gridWidth + gridReactiveData.scrollbarWidth
			}px`;
			refGridBodyLeftFixedTableWrapperDiv.value.style.width = `${gridReactiveData.gridLeftFixedHeaderWidth}px`;
			refGridBodyLeftFixedTableWrapperDiv.value.style.height = `${
			...
			refGridBodyXLineDiv.value.style.width =
				refGridBodyLeftFixedXLineDiv.value.style.width;
			refGridBodyYLineDiv.value.style.height =
				refGridBodyLeftFixedYLineDiv.value.style.height;
			...
			return new Promise((resolve) => {
				...
				if (props.data) {
					if (props.data.type === "map") {
						if (
							props.data.mapData &&
							props.data.mapData.data &&
							props.data.mapData.data.length > 0
						) {
					...
					if (
						props.data.hasOwnProperty("conf") &&
						props.data.conf.hasOwnProperty("merges") &&
						props.data.conf.merges.length > 0
					) {
							...
							if (colStart !== colEnd || rowStart !== rowEnd) {
								...
								Object.keys(gridReactiveData.merges).forEach((key: string) => {
									...
									let startCol = [
										...
										Math.min(
											gridReactiveData.merges[key].colStart,
											gridReactiveData.merges[key].colEnd,
										),
									];
									let endCol = [
									...
									let startRow = [
										...
										Math.min(
											gridReactiveData.merges[key].rowStart,
											gridReactiveData.merges[key].rowEnd,
										),
									];
									let endRow = [
								...
								if (mergeColRows.length === 0) {
									gridReactiveData.merges[
										`${colStart}_${rowStart}:${colEnd}_${rowEnd}`
									] = {
										colStart: colStart,
										colEnd: colEnd,
					...
					if (
						props.data.hasOwnProperty("conf") &&
						props.data.conf.hasOwnProperty("styles")
					) {
					...
					if (
						props.data.hasOwnProperty("conf") &&
						props.data.conf.hasOwnProperty("borders")
					) {
					...
					const columns = [
						...Array<Record<string, unknown>>(
							gridReactiveData.xDim.valueOf() + 1,
						),
					];
					columns.forEach((_, index) => {
						...
						if (
							props.data &&
							props.data.hasOwnProperty("conf") &&
							props.data.conf.hasOwnProperty("colWidth") &&
							props.data.conf.colWidth.length
						) {
							for (let i = 0; i < props.data.conf.colWidth.length; i++) {
								if (
									getColumnCount(props.data.conf.colWidth[i].col) === index &&
									isNumeric(props.data.conf.colWidth[i].width)
								) {
									colWidth = props.data.conf.colWidth[i].width;
									gridReactiveData.columnWidthsChanged[`${index}`] =
										props.data.conf.colWidth[i].width;
									break;
								}
							}
						}
						if (
							props.data &&
							props.data.hasOwnProperty("conf") &&
							props.data.conf.hasOwnProperty("colHide") &&
							props.data.conf.colHide.length
						) {
						...
						gridReactiveData.colConfs.push(
							new Column(index - 1, colWidth || "default", colVisible),
						);
					});

					const rows = [
						...Array<Record<string, unknown>>(gridReactiveData.yDim.valueOf()),
					];
					rows.forEach((_, index) => {
						...
						if (
							props.data &&
							props.data.hasOwnProperty("conf") &&
							props.data.conf.hasOwnProperty("rowHeight") &&
							props.data.conf.rowHeight.length
						) {
							for (let i = 0; i < props.data.conf.rowHeight.length; i++) {
								if (
									props.data.conf.rowHeight[i].row === index + 1 &&
									isNumeric(props.data.conf.rowHeight[i].height)
								) {
									rowHeight = props.data.conf.rowHeight[i].height;
									gridReactiveData.rowHeightsChanged[`${index + 1}`] =
										props.data.conf.rowHeight[i].height;
									break;
								}
							}
						}
						if (
							props.data &&
							props.data.hasOwnProperty("conf") &&
							props.data.conf.hasOwnProperty("rowHide") &&
							props.data.conf.rowHide.length
						) {
						...
						gridReactiveData.rowConfs.push(
							new Row(index, rowHeight || "default", rowVisible),
						);
					});
								...
								if (props.data.type === "map") {
									if (
										...
										cellData = props.data.mapData.data.find(
											(d: any) =>
												getColumnCount(d.p.replace(/[0-9]/g, "")) ===
													colIndex &&
												parseInt(d.p.replace(/[^0-9]/gi, "")) === rowIndex + 1,
										);
									}
								} else {
									if (
										...
										cellData =
											colIndex - 1 < 0
												? null
												: props.data.arrayData[rowIndex][colIndex - 1];
									}
								}
							}

							const { rowSpan, colSpan } = getRowColSpanFromMerges(
								colIndex,
								rowIndex + 1,
								gridReactiveData.merges,
							);
		...
		const $vmaFormulaGrid = {
			...
			getRefs: () => gridRefs,
			renderVN: renderVN,
		} as unknown as VmaFormulaGridConstructor &
			VmaFormulaGridMethods &
			VmaFormulaGridPrivateMethods;
