...
import {
	DragEventOptions,
	VmaFormulaGridCompColorPickerHueConstructor,
} from "../../../types";
	...
	setup(props, context) {
		...
		const onMoveBar = (event: MouseEvent) => {
			...
			if (barElement.value && cursorElement.value) {
				...
				const hue = Math.round(
					((left - offsetWidth / 2) / (rect.width - offsetWidth)) * 360,
				);
				color.hue = hue;
				state.hue = hue;
				emit("change", hue);
		...
		const getCursorLeft = () => {
			if (barElement.value && cursorElement.value) {
				...
				return (
					((state.hue % 360) * (rect.width - offsetWidth)) / 360 +
					offsetWidth / 2
				);
			}
			...
			h(
				...
				h(
					...
					h(
						...
						{
							class: [
								"vma-formula-grid-hue-slider__bar-pointer",
								"small-slider",
							],
							ref: cursorElement,
